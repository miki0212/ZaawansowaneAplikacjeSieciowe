...
const startCounter = (currentIdx: number): void => {
    ...
    let time: number = parseInt(localStorage.getItem(`question-time-${currentIdx}`) || '0', 10);
...
backNode.addEventListener("click", () => {
    const currentIdx: number = parseInt(localStorage.getItem("current-question-idx")!, 10);
